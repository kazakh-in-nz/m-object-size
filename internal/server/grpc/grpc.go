...
import (
	...
	"github.com/kazakh-in-nz/m-object-size/internal/server/logic"
...
type Grpc struct {
	address string
	pbgameengine.UnsafeGameEngineServer
	srv *grpc.Server
}
...
func (g *Grpc) GetSize(ctx context.Context, input *pbgameengine.GetSizeRequest) (*pbgameengine.GetSizeResponse, error) {
	log.Info().Msg("GetSize in m-object-size called")
...
func (g *Grpc) SetScore(ctx context.Context, input *pbgameengine.SetScoreRequest) (*pbgameengine.SetScoreResponse, error) {
	log.Info().Msg("SetScore in m-object size called")
...
func (g *Grpc) ListenAndServe() error {
	...
	log.Info().Str("address", g.address).Msg("starting gRPC server for m-object-size microservice")

	err = g.srv.Serve(lis)

	if err != nil {
		return errors.Wrap(err, "failed to start gRPC server for m-object-size microservice")
	}
